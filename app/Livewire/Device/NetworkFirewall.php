...
use App\Models\AssetFirewall;
use App\Models\Credential;

#[Layout('layouts.contentNavbarLayoutLivewire')]
#[Title('Firewall')]
class NetworkFirewall extends Component
{
  use WithPagination;
  ...
  protected array $messages = [
    ...
    'warranty.date' => 'Warranty harus format tanggal yang valid.',
  ...
  {
    $query = AssetFirewall::query()->with('credential');
  ...
  public function store(): void
  {
    ...
    $this->dispatch('toast', ['type' => 'success', 'message' => 'Firewall berhasil dibuat.']);
  ...
  public function edit($id): void
  {
    ...
    $asset = AssetFirewall::with('credential')->findOrFail($id);
  ...
  {
    if (!$this->id) {
      $this->dispatch('toast', ['type' => 'error', 'message' => 'Tidak ada Firewall yang dipilih untuk update.']);
      return;
    }

    $this->validate();

    $asset = AssetFirewall::findOrFail($this->id);
    $asset->update([
      'hostname' => $this->hostname,
      'type' => $this->type,
      'ip_address' => $this->ip_address,
    ...
    $this->dispatch('asset-modal:close');
    $this->dispatch('toast', ['type' => 'success', 'message' => 'Firewall berhasil diupdate.']);
  ...
  {
    AssetFirewall::findOrFail($id)->delete();

    $this->dispatch('toast', ['type' => 'success', 'message' => 'Firewall berhasil dihapus.']);
    $this->resetPage();
  }
  ...
  public function render()
  {
    ...
    return view('livewire.device.network-firewall', [
      'assets' => $assets,
    ]);
  }
}
