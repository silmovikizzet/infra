...
#[Title('Firewall')]
class NetworkApImporter extends Component
{
  use WithFileUploads;
  ...
  {
    return view('livewire.tools.network-ap-importer');
  }
    ...
    try {
      ...
      foreach ($rows as $r) {
        try {
          ...
          $existing = AssetAp::query()->where($lookup)->first();
          ...
          Log::warning('[AssetAp Import] Row failed', [
            'row' => $rowNumber,
            'error' => $e->getMessage(),
          ]);
        }
      ...
      DB::rollBack();
      Log::error('[AssetAp Import] Fatal', ['error' => $e->getMessage()]);
  ...
  private function buildHeaderMap(array $headerRow): array
  {
    ...
    foreach ($headerRow as $idx => $rawHeader) {
      ...
      $snake = Str::snake($h);
      if (in_array($snake, (new AssetAp())->getFillable(), true)) {
        $map[(int) $idx] = $snake;
      }
    }
  ...
  {
    $fillable = (new AssetAp())->getFillable();
  ...
  private function rowToData(array $row, array $map): array
  {
    ...
    $fillable = array_flip((new AssetAp())->getFillable());
